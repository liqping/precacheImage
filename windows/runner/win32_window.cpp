...
bool Win32Window::Create(const std::wstring& title,
  ...
  HWND window = CreateWindow(
      ...
      //window_class, title.c_str(), WS_VISIBLE  | WS_POPUP | WS_SYSMENU,
  ...
  //db.fEnable = TRUE;
  //DwmEnableBlurBehindWindow(window, &db);
  //BOOL enable;
  //DwmIsCompositionEnabled(&enable);
  //if (!enable) {
  //    auto ss = DwmEnableComposition(DWM_EC_ENABLECOMPOSITION);
  //  
  //    if (ss != S_OK) {
  //        printf("xxx:%d ss:%ld\n",(int)GetLastError(), (HRESULT)(ss));
  //    }
  //    
  //}
  ...
  //MARGINS margins = { -1 };
  //auto result = DwmExtendFrameIntoClientArea(window, &margins);
  //if (SUCCEEDED(result)) {
  //    printf("xxx\n");
  //}


  //SetLayeredWindowAttributes(window, RGB(233,30,99), (BYTE)255, LWA_COLORKEY);
...
LRESULT CALLBACK Win32Window::WndProc(HWND const window,
    ...
    //   auto memBitmap = ::CreateCompatibleBitmap(dc, wndSize.cx, wndSize.cy);

    //   ::SelectObject(memDC, memBitmap);

    //   Rectangle(memDC, 0, 33, 100, 100);
    ...
    //   POINT pt = { 0,0 };
    //   auto ss = UpdateLayeredWindow(window, dc, &ptDest, &wndSize, memDC, &pt, 0, &blendFunction, ULW_ALPHA);
    //   if (!ss) {
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    //    blendFunction.SourceConstantAlpha = 255;
    //    POINT pt = { 0,0 };
    //    auto ss = UpdateLayeredWindow(hwnd, dc, &ptDest, &wndSize, memDC, &pt, 0, &blendFunction, ULW_ALPHA);
    //    if (!ss) {
    //        auto error = GetLastError();
    //        printf("error:%d\n", error);
    //    }
